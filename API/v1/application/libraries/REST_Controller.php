...
abstract class REST_Controller extends CI_Controller
{
	...
	*/
	...
	/**
	 * APi time log start and end will be here 
	 *
	 * @var string
	 */
	protected $_api_time = array();
        
        /**
	 * Last inserting id of logs
	 *
	 * @var string
	 */
	protected $_lastId = '';
	...
	public function __construct()
	{
		...
		// added by rati 
		$this->_api_time['api_time']['start_time'] = date("Y-m-d H:i:s");;
		...
		{
			$this->response(array('status' => false,'error' => 'yes', 'errorMessage' => 'Only AJAX requests are accepted.'), 505);
		}
	}
	...
	public function _remap($object_called, $arguments)
	{
		...
		{
			$this->response(array('status' => false,'error' => 'yes', 'errorMessage' => 'Unsupported protocol'), 403);
		}
		...
		{
			if($this->router->class=="registration" && $this->router->method=="file_upload")
		 	{ }
		 	else
		 	{
				...
				$this->response(array('status' => false,'error' => 'yes', 'errorMessage' => 'Invalid API Key.'), 403);
		 	}
		}
		...
		{
			$this->response(array('status' => false,'error' => 'yes', 'errorMessage' => 'Unknown method.'), 404);
		}
		...
		if (config_item('rest_enable_keys') AND !empty($this->rest->key))
		{
			...
			{
				$this->response(array('status' => false,'error' => 'yes', 'errorMessage' => 'This API key has reached the hourly limit for this method.'), 401);
			}
			...
			$authorized OR $this->response(array('status' => false,'error' => 'yes', 'errorMessage' => 'This API key does not have enough permissions.'), 401);
		}
	...
	{
		// Added By Rati to maitaine API LOG
		$this->_api_time['api_time']['end_time'] = date("Y-m-d H:i:s");;
		$data['api_time'] = $this->_api_time['api_time'];// added by rati 
		
                //------ add response with logs (RB) ----------
                $this->db->where('record_id',$this->_lastId);
                $this->db->update('logs', array('response' => json_encode($data)));
                //-------- end ------------
                $this->api_file_write($data);
		...
		log_message('debug','========= Last output from Response Function ========'.print_r($output,true).'=====');
		exit($output);
	}
	...
	function api_file_write($data,$fileName=''){
		
		$member_id = $this->input->get_post('user_id');
		if(!empty($member_id))
		$fileName = date('Y-m-d');
		else if(empty($file_name) || $file_name = '')
		$fileName = date('Y-m-d').'_'.$this->input->ip_address();
		
		// check folder exist or not if not than create 
		 $folderName = FCPATH.'application/logs/APILOG'.'/';
		...
		if(!empty($member_id))
		{	
			$folderName = FCPATH.'application/logs/APILOG'.'/'.$member_id.'/';
			if(!file_exists($folderName))
		...
		fwrite($fp, $this->input->ip_address()."\n");
		
		fwrite($fp, "\n\n".date('Y-m-d H:i:s')."\n\n");
		fwrite($fp, "POST Data\n");
		fwrite($fp, json_encode($_POST,true));
		fwrite($fp,"\n\n");
		fwrite($fp, "SENDING DATA\n");
		fwrite($fp, json_encode($_GET,true));
		fwrite($fp,"\n\n");
	...
	protected function _log_request($authorized = FALSE)
	{
		if($this->router->class=="registration" && $this->router->method=="file_upload")
		 	...
		 	{$this->rest->db->insert(config_item('rest_logs_table'), array(
                ...
                $this->_lastId = $this->rest->db->insert_id();
                return true;
		 	}
	}
	...
	{
		echo $this->methods[$controller_method]['limit'];die;
	...
	protected function _check_whitelist_auth()
	{
		...
		{
			$this->response(array('status' => false,'error' => 'yes', 'errorMessage' => 'Not authorized'), 401);
		}
	}
