...
//$this->sitter->setAvailabileJobs();
    ...
    public function confirmJob($job_id, $sitter_id, $assigned_by = "")
    {
    	...
    	if($this->jobInfo['job_status']=='pending' || $this->jobInfo['job_status']=='new')
    	{
...
//code to select device token and send push nitifications
        ...
        return true;
    		...
    		return $errormsg="Job has been already ".$this->jobInfo['job_status'];
    	}
    }
...
//for alerts tab
	...
	where is_resolved=0 and ( job_status != 'closed' or (job_status is null and job_logs.job_id=0 )) order by {$orderby['key']} {$orderby['odr']} LIMIT  $start,{$sort['rows']}";
